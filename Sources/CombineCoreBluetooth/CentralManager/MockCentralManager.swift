...
public class MockCentralManager: @unchecked Sendable {
    ...
    func handle(_ handler: @escaping () -> Void) {
        DispatchQueue.main.asyncAfter(deadline: .now() + self.handlingDelay, execute: handler)
    }
    ...
    public init() {
        centralManager = .unimplemented(
            ...
            retrievePeripheralsWithIdentifiers: { uuids in uuids.compactMap({ self.knownPeripherals[$0] }) },
